...
@RestController
@RequestMapping(value = "api/v1/PmsSpuInfo", produces = MediaType.APPLICATION_JSON_VALUE)
public class PmsSpuInfoRestController {

	private static final Logger logger = LoggerFactory.getLogger(PmsSpuInfoRestController.class);

	private PmsSpuInfoService service; // injected
	 ...
	 * @param service
	 */
	protected PmsSpuInfoRestController(PmsSpuInfoService service) {
		super();
	 ...
	 */
	@GetMapping("")
	protected ResponseEntity<List<PmsSpuInfoDTO>> findAll() {
		logger.debug("REST : GET - findAll");
		List<PmsSpuInfoDTO> list = service.findAll();
	 ...
	 */
	@PostMapping("")
	protected ResponseEntity<Void> create(@RequestBody PmsSpuInfoDTO pmsSpuInfoDTO) {
		logger.debug("REST : POST - create");
		if (service.create(pmsSpuInfoDTO)) {
	 ...
	 */
	@PutMapping("/{id}")
	protected ResponseEntity<Void> save(@PathVariable Long id, @RequestBody PmsSpuInfoDTO pmsSpuInfoDTO) {
		logger.debug("REST : PUT - save");
		service.save(id, pmsSpuInfoDTO);
	 ...
	 */
	@PutMapping("")
	protected ResponseEntity<Void> update(@RequestBody PmsSpuInfoDTO pmsSpuInfoDTO) {
		logger.debug("REST : PUT - update");
		if (service.update(pmsSpuInfoDTO)) {
	 ...
	 */
	@PatchMapping("/{id}")
	protected ResponseEntity<Void> partialUpdate(@PathVariable Long id, @RequestBody PmsSpuInfoDTO pmsSpuInfoDTO) {
		logger.debug("REST : PATCH - partialUpdate");
		if (service.partialUpdate(id, pmsSpuInfoDTO)) {
	...
	public ResponseEntity<Void> spuUp(@PathVariable Long spuId) {
		service.upSpu(spuId);
