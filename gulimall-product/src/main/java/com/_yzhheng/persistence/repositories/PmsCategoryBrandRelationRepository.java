/*
 * Created on 2024-05-02 ( 14:45:35 )
 * Generated by Telosys ( https://www.telosys.org/ ) version 3.3.0
 */
package com._yzhheng.persistence.repositories;

import org.springframework.data.jpa.repository.JpaRepository;

import com._yzhheng.persistence.entities.PmsCategoryBrandRelation;

/**
 * Spring Data JPA repository for entity "PmsCategoryBrandRelation" <br> 
 * 
 * This repository extends PagingAndSortingRepository interface <br>
 * so it provides by default all the basic CRUD operations : <br>
 *   findById, findAll, save, delete, etc <br> 
 * with pagination and sorting : <br>
 *   findAll(Pageable), findAll(Sort)<br>
 * 
 * This repository can be extended by adding specific "finders" methods<br>
 * To do so, see the "predicates conventions" for "derived query methods" in Spring Data documentation
 * 
 * @author Telosys
 *
 */
public interface PmsCategoryBrandRelationRepository extends JpaRepository<PmsCategoryBrandRelation, Long> {

	// Insert specific finders here 

	//List<PmsCategoryBrandRelation> findByXxx(String xxx);

	//List<PmsCategoryBrandRelation> findByXxxStartingWith(String xxx);

	//List<PmsCategoryBrandRelation> findByXxxContaining(String xxx);

	//List<PmsCategoryBrandRelation> findByYyy(BigDecimal yyy);

	//List<PmsCategoryBrandRelation> findByXxxContainingAndYyy(String xxx, BigDecimal yyy);
}
