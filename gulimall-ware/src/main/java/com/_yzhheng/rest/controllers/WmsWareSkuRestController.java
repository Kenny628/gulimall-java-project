...
public class WmsWareSkuRestController {

	private static final Logger logger = LoggerFactory.getLogger(WmsWareSkuRestController.class);

	private WmsWareSkuService service; // injected
	 ...
	 * @param service
	 */
	protected WmsWareSkuRestController(WmsWareSkuService service) {
		super();
	 ...
	 */
	@GetMapping("")
	protected ResponseEntity<List<WmsWareSkuDTO>> findAll() {
		logger.debug("REST : GET - findAll");
		List<WmsWareSkuDTO> list = service.findAll();
	 ...
	 */
	@GetMapping("/{id}")
	protected ResponseEntity<WmsWareSkuDTO> findById(@PathVariable Long id) {
		logger.debug("REST : GET - findById");
		WmsWareSkuDTO wmsWareSkuDTO = service.findById(id);
		if (wmsWareSkuDTO != null) {
			return ResponseEntity.ok(wmsWareSkuDTO); // 200 OK, found
		} else {
	 ...
	 */
	@PostMapping("")
	protected ResponseEntity<Void> create(@RequestBody WmsWareSkuDTO wmsWareSkuDTO) {
		logger.debug("REST : POST - create");
		if (service.create(wmsWareSkuDTO)) {
	 ...
	 */
	@PutMapping("/{id}")
	protected ResponseEntity<Void> save(@PathVariable Long id, @RequestBody WmsWareSkuDTO wmsWareSkuDTO) {
		logger.debug("REST : PUT - save");
		service.save(id, wmsWareSkuDTO);
	 ...
	 */
	@PutMapping("")
	protected ResponseEntity<Void> update(@RequestBody WmsWareSkuDTO wmsWareSkuDTO) {
		logger.debug("REST : PUT - update");
		if (service.update(wmsWareSkuDTO)) {
	 ...
	 */
	@PatchMapping("/{id}")
	protected ResponseEntity<Void> partialUpdate(@PathVariable Long id, @RequestBody WmsWareSkuDTO wmsWareSkuDTO) {
		logger.debug("REST : PATCH - partialUpdate");
		if (service.partialUpdate(id, wmsWareSkuDTO)) {
